...
public class GameEngine_Rev {
    ...
    private final GameField_Rev field;
     ...
     */
    public GameEngine_Rev() {
        this.enemies = new ArrayList<>();
        this.bullets = new ArrayList<>();
        this.spaceShip = new SpaceShip_Rev(WIDTH / 2, HEIGHT - 1, SPACE_SHIP_HEALTH);
        this.field = new GameField_Rev(spaceShip, enemies, bullets);
    }
     ...
     */
    public void run() {
        Drawer_Rev.printControls();

        boolean running = true;
        while (running) {
            Drawer_Rev.render(spaceShip, enemies, bullets, field.getScore());
            ...
            System.out.print("Move (A/D), Shoot (S), Shoot Infinity Bullet (W), Wait (Enter), Quit (Q): ");
    ...
    private void update(char command) {
        switch (command) {
            ...
            case 'S':
                ...
                break;
            case 'W':
                bullets.add(spaceShip.shootInfinity());
            default:
                break;
        }

        for (Enemy_Rev e : enemies) {
            e.update();
        }

        for (Bullet_Rev b : bullets) {
            b.update();
        }
