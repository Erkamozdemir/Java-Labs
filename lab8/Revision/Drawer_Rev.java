...
public class Drawer_Rev {
     ...
     */
    public static void render(SpaceShip_Rev spaceShip, List<Enemy_Rev> enemies, List<Bullet_Rev> bullets, int score) {
        // Create an empty 2D character grid
        char[][] grid = new char[GameEngine_Rev.HEIGHT][GameEngine_Rev.WIDTH];
        for (char[] row : grid) {
            Arrays.fill(row, ' ');
        }
    ...
    private static void printField(char[][] grid) {
        // Top border
        for (int i = 0; i < GameEngine_Rev.WIDTH + 2; i++) {
            System.out.print('=');
        }
        System.out.println();
